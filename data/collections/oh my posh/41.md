...
## Repaint the prompt on demand [​](https://ohmyposh.dev/docs/faq\#repaint-the-prompt-on-demand "Direct link to Repaint the prompt on demand")
...
## My posh-git prompt string doesn't render anymore [​](https://ohmyposh.dev/docs/faq\#my-posh-git-prompt-string-doesnt-render-anymore "Direct link to My posh-git prompt string doesn't render anymore")

You're relying on the `POSH_GIT_STRING` environment variable, set via `Write-GitStatus` and shown through a
[text segment](https://ohmyposh.dev/docs/segments/system/text), and instead of your git status you now see raw escape codes like `[93m[[39m...`.\
\
...
\
Migrate to the git segment's `source: pwsh` option instead. It fetches the same data from the [posh-git](https://github.com/dahlbyk/posh-git)\
module, but through the segment's regular template properties (`.Staging`, `.Working`, `.BranchStatus`, ...)\
rather than a pre-rendered, ANSI-laden string, so nothing gets stripped. See the [posh-git section](https://ohmyposh.dev/docs/segments/scm/git#posh-git)\
...
- [Repaint the prompt on demand](https://ohmyposh.dev/docs/faq#repaint-the-prompt-on-demand)\
...
- [My posh-git prompt string doesn't render anymore](https://ohmyposh.dev/docs/faq#my-posh-git-prompt-string-doesnt-render-anymore)\
