[Skip to main content](https://ohmyposh.dev/docs/configuration/data#__docusaurus_skipToContent_fallback)
...
## File format [​](https://ohmyposh.dev/docs/configuration/data\#file-format "Direct link to File format")
...
[segments.git.Working]

Added = 2

Modified = 1

[segments.git.Staging]
...
## The `env` section [​](https://ohmyposh.dev/docs/configuration/data\#the-env-section "Direct link to the-env-section")

Keys are the [global template properties](https://ohmyposh.dev/docs/configuration/templates#global-properties) you already use in templates (`PWD`, `AbsolutePWD`,
...
The [`maps`](https://ohmyposh.dev/docs/configuration/general#maps) aliases (`user_name`, `host_name`, `shell_name`) apply to data values as they do to live
ones.

### Precedence [​](https://ohmyposh.dev/docs/configuration/data\#precedence "Direct link to Precedence")

`PWD`, `Code`, `ExecutionTime`, and `PipeStatus` feed the runtime itself, not just the template context. Live
segments that compute from them, such as [path](https://ohmyposh.dev/docs/segments/system/path), [status](https://ohmyposh.dev/docs/segments/system/status), and [executiontime](https://ohmyposh.dev/docs/segments/system/executiontime),
...
## The `segments` section [​](https://ohmyposh.dev/docs/configuration/data\#the-segments-section "Direct link to the-segments-section")
...
A few segments store a property under a different name than the template exposes. The [wakatime](https://ohmyposh.dev/docs/segments/web/wakatime) segment, for
example, stores `.CumulativeTotal` as `cumulative_total`. When a property you wrote by hand has no effect, run
[`config export data`](https://ohmyposh.dev/docs/configuration/data#recording-data) and copy the names from its output: the recorder always uses the
stored names.

A segment listed here is force-enabled: it renders even where its own live check would otherwise suppress it,
for example a [battery](https://ohmyposh.dev/docs/segments/system/battery) segment on a machine without one. A segment left out of the file executes
...
- **A [recorded](https://ohmyposh.dev/docs/configuration/data#recording-data) file** carries a `version` marker and replays a listed segment from the
...
[segments.PathSecondary]

Path = "~/dev/site"
```

## Template and method fidelity [​](https://ohmyposh.dev/docs/configuration/data\#template-and-method-fidelity "Direct link to Template and method fidelity")

Templates evaluate over the data you supply, so template methods and functions work as expected.
`.Working.String` on the [git](https://ohmyposh.dev/docs/segments/scm/git) segment, for example, computes from the `Working` property in the file,
just as it would from a live repository. Only logic that reads live runtime state at render time can't be
reproduced this way. This matches how a [cached](https://ohmyposh.dev/docs/configuration/segment#cache) segment behaves: it also renders from stored
properties instead of re-detecting them.

## Recording data [​](https://ohmyposh.dev/docs/configuration/data\#recording-data "Direct link to Recording data")
...
[hand-written form](https://ohmyposh.dev/docs/configuration/data#the-segments-section) with bare properties keeps working.
...
### Deriving instead of replaying a recorded file [​](https://ohmyposh.dev/docs/configuration/data\#deriving-instead-of-replaying-a-recorded-file "Direct link to Deriving instead of replaying a recorded file")

`--data-derive` applies the [hand-written derive behavior](https://ohmyposh.dev/docs/configuration/data#the-segments-section) to a recorded file, running
...
## End-to-end example [​](https://ohmyposh.dev/docs/configuration/data\#end-to-end-example "Direct link to End-to-end example")
...
## Exporting an image [​](https://ohmyposh.dev/docs/configuration/data\#exporting-an-image "Direct link to Exporting an image")
...
- [File format](https://ohmyposh.dev/docs/configuration/data#file-format)
- [The `env` section](https://ohmyposh.dev/docs/configuration/data#the-env-section)
  - [Precedence](https://ohmyposh.dev/docs/configuration/data#precedence)
- [The `segments` section](https://ohmyposh.dev/docs/configuration/data#the-segments-section)
- [Template and method fidelity](https://ohmyposh.dev/docs/configuration/data#template-and-method-fidelity)
- [Recording data](https://ohmyposh.dev/docs/configuration/data#recording-data)
  - [Deriving instead of replaying a recorded file](https://ohmyposh.dev/docs/configuration/data#deriving-instead-of-replaying-a-recorded-file)
- [End-to-end example](https://ohmyposh.dev/docs/configuration/data#end-to-end-example)
- [Exporting an image](https://ohmyposh.dev/docs/configuration/data#exporting-an-image)